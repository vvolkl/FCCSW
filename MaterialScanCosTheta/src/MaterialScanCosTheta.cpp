#include "MaterialScanCosTheta.h"
...
MaterialScanCosTheta::MaterialScanCosTheta(const std::string& name, ISvcLocator* svcLoc) : Service(name, svcLoc),
m_geoSvc("GeoSvc", name) {}

StatusCode MaterialScanCosTheta::initialize() {
  ...
  sc = m_flatEtaDist.initialize(randSvc, Rndm::Flat(0., m_cosThetaBinning));
  if (sc == StatusCode::FAILURE) {
    error() << "Unable to initialize random number generator." << endmsg;
    return sc;
  }
  ...
  TH1F nX0Hist = TH1F("nX0", ";cos #theta;Number of X_{0}", (int) 2* m_cosThetaMax / m_cosThetaBinning , -m_cosThetaMax, m_cosThetaMax);
  TH1F matDepthHist = TH1F("matDepth", ";cos #theta; Material Depth [cm]", (int) 2* m_cosThetaMax / m_cosThetaBinning , -m_cosThetaMax, m_cosThetaMax);
  TH1F nLambdaHist= TH1F("nLambda", ";cos #theta;Number of #lambda", (int) 2* m_cosThetaMax / m_cosThetaBinning , -m_cosThetaMax, m_cosThetaMax);
  ...
  TVector3 vec(0, 0, 0);
  int ibin = 0;
  for (eta = -m_cosThetaMax; eta < m_cosThetaMax; eta += m_cosThetaBinning) {
    nX0->clear();
    nLambda->clear();
    matDepth->clear();
    material->clear();
    ...
    for (int iPhi = 0; iPhi < m_nPhiTrials; ++iPhi) {
      ...
      vec.SetPtThetaPhi(1, std::acos(etaRndm), phi);
      auto n = vec.Unit();
      dir = {n.X(), n.Y(), n.Z()};
    ...
    for (auto matpair : phiAveragedMaterialsBetween) {
      ...
      l_matDepth += matpair.second;
      l_nX0 += matpair.second / mat->GetRadLen();
      l_nLambda += matpair.second / mat->GetIntLen();


    }
    matDepthHist.SetBinContent(ibin+1, l_matDepth);
    nX0Hist.SetBinContent(ibin+1, l_nX0);
    nLambdaHist.SetBinContent(ibin+1, l_nLambda);
    matDepth->push_back(l_matDepth);
    nX0->push_back(l_nX0);
    nLambda->push_back(l_nLambda);
    tree->Fill();
    ibin++;
  }
  matDepthHist.Write();
...
StatusCode MaterialScanCosTheta::finalize() { return StatusCode::SUCCESS; }

DECLARE_SERVICE_FACTORY(MaterialScanCosTheta)
