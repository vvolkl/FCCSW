...
DECLARE_TOOL_FACTORY(TowerToolBarrel)

TowerToolBarrel::TowerToolBarrel(const std::string& type, const std::string& name, const IInterface* parent)
...
StatusCode TowerToolBarrel::initialize() {
...
StatusCode TowerToolBarrel::finalize() { return GaudiTool::finalize(); }

std::pair<double, double> TowerToolBarrel::retrievePhiEtaExtrema(dd4hep::DDSegmentation::Segmentation* aSegmentation, SegmentationType aType) {
  double phiMax = -1;
  double etaMax = -1;
...
tower TowerToolBarrel::towersNumber() {
...
uint TowerToolBarrel::buildTowers(std::vector<std::vector<float>>& aTowers) {
...
uint TowerToolBarrel::idEta(float aEta) const {
  uint id = floor((aEta + m_etaMax) / m_deltaEtaTower);
  return id;
}

uint TowerToolBarrel::idPhi(float aPhi) const {
  uint id = floor((aPhi + m_phiMax) / m_deltaPhiTower);
  return id;
}

float TowerToolBarrel::eta(int aIdEta) const {
  // middle of the tower
  return ((aIdEta + 0.5) * m_deltaEtaTower - m_etaMax);
}

float TowerToolBarrel::phi(int aIdPhi) const {
  // middle of the tower
  return ((aIdPhi + 0.5) * m_deltaPhiTower - m_phiMax);
}

uint TowerToolBarrel::phiNeighbour(int aIPhi) const {
  if (aIPhi < 0) {
...
float TowerToolBarrel::radiusForPosition() const { return m_radius; }

void TowerToolBarrel::CellsIntoTowers(std::vector<std::vector<float>>& aTowers,
...
std::pair<dd4hep::DDSegmentation::Segmentation*, TowerToolBarrel::SegmentationType> TowerToolBarrel::retrieveSegmentation(std::string aReadoutName) {
...
void TowerToolBarrel::attachCells(float eta, float phi, uint halfEtaFin, uint halfPhiFin, fcc::CaloCluster& aEdmCluster, bool aEllipse) {
  int etaId = idEta(eta);
  int phiId = idPhi(phi);
  int num1 = 0;
  int num2 = 0;
