...
import ROOT
ylim= {'depth': 250, 'x0': 70, 'lambda':8}
material_order = ["Aluminum", "LAr", "Lead", "Copper", "PCB", "lArCaloGlue", "lArCaloSteel"]
...
def main():
    ...
    parser.add_argument('--ylim', "-y", dest='ylim', default=-1, type=float, help="y lim")
    ...
    for etaBin, entry in enumerate(tree):
        ...
        # create entries with given order
        for j, mat in enumerate(material_order):
          for i in range(nMat):
            if entry.material.at(i) == mat:
    ...
    for plot, title in zip(["x0", "lambda", "depth"], axis_titles):
        ...
        ths.Draw()
        ths.GetYaxis().SetRangeUser(0,100)# ylim[plot])
        ths.GetXaxis().SetTitle("#eta")
        ths.GetYaxis().SetTitle(title)

        legend.Draw()
        cv.Print(args.fname.replace(".root", "_") + plot + ".pdf")

        ths.GetXaxis().SetRangeUser(0, args.etaMax)
        cv.Print(args.fname.replace(".root", "_") +plot + "pos.pdf")
